...
async def consume(topic_name):
    ...
    while True:
        ...
        if message is None:
            ...
            print("error: {}".format(message.error()))  # Error has `str` type
        else:
            print("{} => {}".format(message.key(), message.value()))
        #
        await asyncio.sleep(1)
...
def main():
    ...
    client.create_topics([NewTopic(TOPIC_NAME, num_partitions=1, replication_factor=1)])
